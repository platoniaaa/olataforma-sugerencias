...
- **InStock (repuestos de pauta):** los repuestos de las pautas de mantención de **Ford Transit, Ranger y F-150 (años modelo 2023 y 2024)** y de **Hyundai Accent, Grand i10 e i20 (todas sus generaciones)** no pueden faltar en las sucursales con taller (Linderos, Rancagua, Curicó y Chillán): si entre stock, tránsito y sugerido no llegan a 2 unidades, el sugerido se completa hasta esa cifra aunque el modelo no pida nada, y la fila se muestra igual con "solo pedir" activado. La columna **InStock** marca esos repuestos en toda la grilla; el mínimo solo obliga en las cuatro sucursales con taller. No entran lubricantes ni consumibles (aceite, fluido de frenos, refrigerante): se venden por litro y un mínimo de "2 unidades" no significa nada sobre ellos. La lista sale de las pautas del fabricante (`scripts/extraer_instock_pautas.py` → `src/data/pautas_instock.csv` → `python -m src.jobs.cargar_instock`).
